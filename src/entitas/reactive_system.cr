...
module Entitas
  ...
  abstract class ReactiveSystem
    ...
    def activate
      {% if flag?(:entitas_enable_logging) %}Log.info { "activating collector : #{self.collector}" }{% end %}
    ...
    # `ReactiveSystem` are activated by default.
    def deactivate
      {% if flag?(:entitas_enable_logging) %}Log.info { "deactivating collector : #{self.collector}" }{% end %}
      self.collector.deactivate
    end

    # Clears all accumulated changes.
    def clear
      {% if flag?(:entitas_enable_logging) %}Log.info { "clearing collector : #{self.collector}" }{% end %}
